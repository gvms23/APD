...
namespace APD.Util
{
    ...
    public class BlockingMultiplexer<T> 
    {
        ...
        private struct ProducerInfo
        {
            ...
            public BlockingCollection<T> Collection;  // producer's fila
        ...
        public BlockingCollection<T> GetProducerFila()
        {
            ...
            lock (producersLock)
            {
                if (mode != Mode.Starting)
                    throw new InvalidOperationException("Cannot get new producer fila for running multiplexer");
        ...
        public IEnumerable<T> GetConsumingEnumerable(CancellationToken token)
        {
            ...
            while (!complete || producers.Any(info => info.HasPendingValue))
            {
                ...
                // values to read from the producer filas. This medias that producer blocking collections 
                ...
                else
                {
                    while (!complete)
                    {
                        ...
                        {
                            throw new InvalidOperationException("Blocking fila delivered duplicate sequence number to multiplexer (1). The duplicate value is "
                                + lockId.ToString());
                        }
                        else if (lockId <= producers[index].LastLockId)
                        {
                            throw new InvalidOperationException("Blocking fila delivered out-of-order item (2)");
                        }
                        else if (producers.Where(info => info.HasPendingValue)
                                           .Any(info => info.PendingLockId == lockId))
                        {
                            throw new InvalidOperationException("Blocking fila delivered duplicate sequence number to multiplexer (2)");
                        }
                        else
                        {
#if DEBUG
